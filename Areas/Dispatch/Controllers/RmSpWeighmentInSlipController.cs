...
{
	[Area("Dispatch")]
	public class RmSpWeighmentInSlipController : BaseController<ResponseModel<WeighmentInSlip>>
	{
		#region Loading
		...
		public IActionResult Load_PO_Dtls(string Vehicle_No = "", long Gate_In_Out_Id = 0, bool IsPrint = false)
		{
			...
			if (!string.IsNullOrEmpty(Vehicle_No) || Gate_In_Out_Id > 0)
			{
				try
				{
					...
					DataSet ds = DataContext.ExecuteStoredProcedure_DataSet_SQL("PC_RM_WEIGHMENT_IN_SLIP_GET", oParams);
					...
					if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
					{
						...
						{
							Report_Title = "WEIGHMENT SLIP RAW MATERIAL - WEIGH IN PRINT",
							...
							Tare_Wt = dt.Rows[0]["GROSS_WT"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["GROSS_WT"]) : "",
							Tare_Wt_Dt = dt.Rows[0]["GROSS_WT_DT"] != DBNull.Value ? DateTime.ParseExact(Convert.ToString(dt.Rows[0]["GROSS_WT_DT"]), "dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture) : nullDateTime,
		...
		public IActionResult Load_SO_Dtls(string Vehicle_No = "", long Gate_In_Out_Id = 0, bool IsPrint = false)
		{
			...
			if (!string.IsNullOrEmpty(Vehicle_No) || Gate_In_Out_Id > 0)
			{
				try
				{
					...
					DataSet ds = DataContext.ExecuteStoredProcedure_DataSet_SQL("PC_SP_WEIGHMENT_IN_SLIP_GET", oParams);
					...
					if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
					{
						...
						obj = new WeighmentInSlip()
						{
							Report_Title = "WEIGHMENT SLIP SCRAP - WEIGH IN PRINT",
							Plant_Name = dt.Rows[0]["PLANT_NAME"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["PLANT_NAME"]) : "",
							Plant_Address = dt.Rows[0]["PlantAddress"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["PlantAddress"]) : "",
							MDA_No = dt.Rows[0]["COMMON_NO"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["COMMON_NO"]) : "",
							Truck_No = dt.Rows[0]["Truck_No"] != DBNull.Value ? Convert.ToString(dt.Rows[0]["Truck_No"]) : "",
		...
		[Route("RmSpWeighmentInSlip/Export/{Vehicle_No?}")]
