	...
	public class GateOutController : BaseController<ResponseModel<GateIn>>
	{
		...
		public IActionResult GetMDA(string type, string searchTerm, int Inward_Sys_Id)
		{
			...
			try
			{
				...
				if (Inward_Sys_Id == 1)
				{
					dt = DataContext.ExecuteStoredProcedure_DataTable_SQL("PC_GATE_OUT_GET", oParams, true);
					...
					if (dt != null && dt.Rows.Count > 0)
						foreach (DataRow dr in dt.Rows)
							list.Add(new GateIn()
							{
								...
								TransactionType = Inward_Sys_Id == 4 && dr["TRANSACTION_TYPE"] != DBNull.Value ? Convert.ToString(dr["TRANSACTION_TYPE"]) : "",
				...
				{
					dt = DataContext.ExecuteStoredProcedure_DataTable_SQL("PC_GATE_OUT_GET_OTHER", oParams, true);
					...
					if (dt != null && dt.Rows.Count > 0)
						foreach (DataRow dr in dt.Rows)
							list.Add(new GateIn()
							{
								...
								WeighOut_Wt = dr["WeighOut_Wt"] != DBNull.Value ? Convert.ToDouble(dr["WeighOut_Wt"]) : 0,
								WeighOut_Wt_Dt = dr["WeighOut_Wt_Dt"] != DBNull.Value ? Convert.ToString(dr["WeighOut_Wt_Dt"]) : ""
							});
				}
