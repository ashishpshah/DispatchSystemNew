...
namespace Dispatch_System.Areas.Admin.Controllers
{
    ...
    public class Load_MDAController : BaseController<ResponseModel<MDA>>
    {
        ...
        [HttpGet]
        public IActionResult Delete_QR_Code(string qr_code = null, string qr_code_type = null, long mda_id = 0, long mda_dtls_id = 0, long gatein_id = 0, long prod_id = 0)
        {
            long requiredShipper = 0;
            long loaddedShipper = 0;
            long rejectShipper = 0;
            ...
            try
            {
                if (gatein_id > 0 && mda_id > 0 && !string.IsNullOrEmpty(qr_code))
                {
                    //var arrayQrCode = qr_codes.Split('|');

                    //if (arrayQrCode != null && arrayQrCode.Length > 0)
                    //{
                    List<MySqlParameter> oParams = new List<MySqlParameter>();
                    ...
                    //oParams.Add(new MySqlParameter("P_QR_CODE_IDS_S", MySqlDbType.VarString) { Value = arrayQrCode.Length > 0 && arrayQrCode.Any(x => x.Contains("S")) ? string.Join("|", arrayQrCode.Where(x => x.Contains("S")).Select(x => x.Split(',')[0]).ToArray()) : "" });
                    //oParams.Add(new MySqlParameter("P_QR_CODE_IDS_R", MySqlDbType.VarString) { Value = arrayQrCode.Length > 0 && arrayQrCode.Any(x => x.Contains("R")) ? string.Join("|", arrayQrCode.Where(x => x.Contains("R")).Select(x => x.Split(',')[0]).ToArray()) : "" });
