...
{
    [Area("Admin")]
    public class DesignationController : BaseController<ResponseModel<Designation>>
    {
        #region Loading
        public IActionResult Index()
        {
            try
            {
                CommonViewModel.ListObj = new List<Designation>();
                ...
                if (AppHttpContextAccessor.IsCloudDBActive)
                {
                    ...
                    dt = DataContext.ExecuteStoredProcedure_DataTable("PC_DESIGNATION_GET", oParams, true);
                ...
                else
                {
                    ...
                    dt = DataContext.ExecuteStoredProcedure_DataTable_SQL("PC_DESIGNATION_GET", oParams, true);
                }

                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                        CommonViewModel.ListObj.Add(new Designation()
                        {
        ...
        public JsonResult Save(WorkStation viewModel)
        {
            try
            {
                ...
                if (AppHttpContextAccessor.IsCloudDBActive)
                {
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure("PC_DESIGNATION_SAVE", oParams, true);
                }
                else
                {
                    List<MySqlParameter> oParams = new List<MySqlParameter>();
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure_SQL("PC_DESIGNATION_SAVE", oParams, true);
                }
        ...
        public JsonResult DeleteConfirmed(long id = 0)
        {
            try
            {
                ...
                if (AppHttpContextAccessor.IsCloudDBActive)
                {
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure("PC_DESIGNATION_DELETE", oParams, true);
                }
                else
                {
                    List<MySqlParameter> oParams = new List<MySqlParameter>();
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure_SQL("PC_DESIGNATION_DELETE", oParams, true);
                }
