	...
	public class CancleGateInController : BaseController<ResponseModel<GateIn>>
	{
		...
		public IActionResult GetMDA(string type, string searchTerm)
		{
			...
			try
			{
				...
				dt = DataContext.ExecuteStoredProcedure_DataTable_SQL("PC_EXP_GATE_IN_CANCEL_GET", oParams, true);
		...
		public IActionResult Save(GateIn viewModel)
		{
			try
			{
				...
				var (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure_SQL("PC_EXP_GATE_IN_CANCEL_SAVE", oParams, true);
