...
{
    [Area("Admin")]
    public class MDADispatchShippersListController : BaseController<ResponseModel<MDA_Status>>
    {
        ...
        public ActionResult GetData_MDADispatchShippersList(JqueryDatatableParam param)
        {   
            ...
            var dt = DataContext.ExecuteStoredProcedure_DataTable("PC_MDA_DISPATCH_SHIPPER_REPORT", oParams, true);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                    result.Add(new MDA_Status
                    {
                        ...
                        ShipperBatchNo = dr["BATCHNO"] != DBNull.Value ? Convert.ToString(dr["BATCHNO"]) : "",
                    });
            }
        ...
        [HttpGet]
        public IActionResult GetData(string Mda_No, bool withDetail = false, bool isPrint = false)
        {
            ...
            try
            {
                ...
                ds = DataContext.ExecuteStoredProcedure_DataSet("PC_MDA_DISPATCH_SHIPPER_REPORT_NEW", oParams);
            ...
            dynamic objFilter = new { Mda_No = Mda_No };
