...
{
	[Area("Admin")]
	public class VendorPoVsQrCodeGenController : BaseController<ResponseModel<QRCodeGeneration>>
	{
		...
		public ActionResult GetData_QrCodeViewSup(JqueryDatatableParam param)
		{
			...
			oParams.Add(new OracleParameter("P_PROD_ID", OracleDbType.Int64) { Value = 0 });
			...
			var dt = DataContext.ExecuteStoredProcedure_DataTable("PC_REPORT_VENDOR_VS_QRCODE", oParams);
			...
			if (dt != null && dt.Rows.Count > 0)
			{
				foreach (DataRow dr in dt.Rows)
					result.Add(new QRCodeGeneration()
					{
						...
						PO_Date_Text = dr["PO_DATE_TEXT"] != DBNull.Value ? Convert.ToString(dr["PO_DATE_TEXT"]) : "",
        ...
        [HttpGet]
        public IActionResult GetData(string PO_Number, bool withDetail = false, bool isPrint = false)
        {
            ...
            try
            {
                ...
                ds = DataContext.ExecuteStoredProcedure_DataSet("PC_REPORT_VENDOR_VS_QRCODE", oParams);
            ...
            if (!string.IsNullOrEmpty(PO_Number))
                PageTitle_Secondary += PO_Number.ToUpper();

            dynamic objFilter = new { PO_Number = PO_Number};
