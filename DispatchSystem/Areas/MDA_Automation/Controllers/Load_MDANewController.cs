					...
					if (dt != null && dt.Rows.Count > 0)
					{
						...
						if (viewModel.Required_Shipper > viewModel.Loaded_Shipper)
						{
								...
								&& IPAddress.TryParse(listenIPString, out listenIP) && int.TryParse(listenPortString, out listenPort) && listenPort > 0 && listenPort <= 65535)
							{
								...
								Task.Run(() => _socketBackgroundTask.StartWork(listenIPString, listenPortString, AppHttpContextAccessor.Loading_Bay, MDA_QR_Scan_Log_IsActive, MDA_QR_Scan_Response_Demo, MDA_QR_Scan_Delay_Sec, filePath, iffco_url)).Wait();
							}
							else
							{
								CommonViewModel.IsConfirm = false;
			...
			try
			{
				...
				if (viewModel.GateInOut_Id > 0 && viewModel.Mda_Id > 0)
				{
					...
					if (dt != null && dt.Rows.Count > 0)
					{
						...
						_socketBackgroundTask.SendToPrinter(dt.Rows[0]["MDA_ORDER"].ToString(), dt.Rows[0]["PARTY_NAME"].ToString(), dt.Rows[0]["desp_place"].ToString(), listenIPString, listenPortString, AppHttpContextAccessor.Loading_Bay);
