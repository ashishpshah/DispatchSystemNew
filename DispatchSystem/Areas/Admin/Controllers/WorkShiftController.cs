...
namespace Dispatch_System.Areas.Admin.Controllers
{
    ...
    public class WorkShiftController : BaseController<ResponseModel<WorkShift>>
    {
        ...
        public IActionResult Index()
        {
            try
            {
                CommonViewModel.ListObj = new List<WorkShift>();
                ...
                if (AppHttpContextAccessor.IsCloudDBActive)
                {
                    ...
                    dt = DataContext.ExecuteStoredProcedure_DataTable("PC_WORK_SHIFT_GET", oParams, true);
                }
                else
                {
                    ...
                    dt = DataContext.ExecuteStoredProcedure_DataTable_SQL("PC_WORK_SHIFT_GET", oParams, true);
                }

                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                        CommonViewModel.ListObj.Add(new WorkShift()
                        {
        ...
        public JsonResult Save(WorkStation viewModel)
        {
            try
            {
                ...
                if (AppHttpContextAccessor.IsCloudDBActive)
                {
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure("PC_WORK_SHIFT_SAVE", oParams, true);
                }
                else
                {
                    List<MySqlParameter> oParams = new List<MySqlParameter>();
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure_SQL("PC_WORK_SHIFT_SAVE", oParams, true);
                }
        ...
        public JsonResult DeleteConfirmed(long id = 0)
        {
            try
            {
                ...
                if (AppHttpContextAccessor.IsCloudDBActive)
                {
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure("PC_WORK_SHIFT_DELETE", oParams, true);
                }
                else
                {
                    List<MySqlParameter> oParams = new List<MySqlParameter>();
                    ...
                    (IsSuccess, response, Id) = DataContext.ExecuteStoredProcedure_SQL("PC_WORK_SHIFT_DELETE", oParams, true);
                }
