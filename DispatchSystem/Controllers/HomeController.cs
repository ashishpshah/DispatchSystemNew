					...
					foreach (var sourceFilePath in orderedFilePaths.Where(x => !x.Trim().Contains("_Error") && !destinationFilePaths.Any(y => y.StartsWith(x))).OrderBy(x => x).ToList())
					{
														...
														foreach (DataRow dr in table_RowsInSqlNotInOracle.Rows)
														{
															if ((dr["SHIPPER_QRCODE_SYSID"] != DBNull.Value ? Convert.ToInt64(dr["SHIPPER_QRCODE_SYSID"]) : 0) > 0)
															{
																sqlQuery_Select += $"SELECT {(dr["SHIPPER_QRCODE_SYSID"] != DBNull.Value ? Convert.ToInt64(dr["SHIPPER_QRCODE_SYSID"]) : 0)} SHIPPER_QRCODE_SYSID" +
																	...
																	$", STR_TO_DATE('{(dr["EVENTTIME"] != DBNull.Value ? Convert.ToDateTime(dr["EVENTTIME"]) : DateTime.MinValue).ToString("dd-MM-yyyy HH:mm:ss").Replace("/", "-")}', '%d-%m-%Y %H:%i:%s') EVENTTIME" +
																	$", 1, NOW() " +
																	$" FROM DUAL UNION ";
															}
														}
													...
													foreach (DataRow dr in table_RowsInSqlNotInOracle_Shipper.Rows)
													{
														if ((dr["SHIPPER_QRCODE_SYSID"] != DBNull.Value ? Convert.ToInt64(dr["SHIPPER_QRCODE_SYSID"]) : 0) > 0)
														{
															sqlQuery_Select += $"SELECT {(dr["SHIPPER_QRCODE_SYSID"] != DBNull.Value ? Convert.ToInt64(dr["SHIPPER_QRCODE_SYSID"]) : 0)} SHIPPER_QRCODE_SYSID" +
																...
																$", TO_DATE('{(dr["EVENTTIME"] != DBNull.Value ? Convert.ToDateTime(dr["EVENTTIME"]) : DateTime.MinValue).ToString("dd-MM-yyyy HH:mm:ss").Replace("/", "-")}', 'DD-MM-YYYY HH24:MI:SS') EVENTTIME " +
																$" FROM DUAL UNION ";
														}
													}
						...
						catch (JsonReaderException ex)
						{
							...
							if (ex != null)
							{
								...
								Write_Log($"File(s) Error at {DateTime.Now.ToString("dd-MM-yyyy hh:mm:ss tt").Replace("/", "-")} : {_error + Environment.NewLine}", logFilePath);
