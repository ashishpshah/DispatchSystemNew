	...
	public class HomeController : BaseController<ResponseModel<LoginViewModel>>
	{
		...
		public IActionResult SyncBatch_New(string searchTerm = null)
		{
			try
			{
				...
				if (sourceFilePaths != null && sourceFilePaths.Count() > 0)
				{
					...
					foreach (var sourceFilePath in orderedFilePaths.Where(x => !x.Trim().Contains("_Error") && !destinationFilePaths.Any(y => y.StartsWith(x))).OrderBy(x => x).ToList())
					{
						...
						Write_Log((string.IsNullOrEmpty(error) ? "" : "Error : " + error + Environment.NewLine) + Environment.NewLine + $"File Processing With Out Validation {Path.GetFileName(sourceFilePath)} " +
							$"Completed at {DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt").Replace("-", "/")}", logFilePath);
					}
				}
				else
				{
				...
				Write_Log(Environment.NewLine + $"File(s) Processing Completed at {DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt").Replace("-", "/")}" + Environment.NewLine, logFilePath);
		...
		public IActionResult SyncBatch()
		{
			try
			{
						...
						try
						{
							...
							if (string.IsNullOrEmpty(error) && shipperData != null && (shipperData.ShipperQRCode_Data != null || shipperData.ShipperQRCode_Data.Count() > 0))
							{
								...
								if (string.IsNullOrEmpty(error))
								{
									...
									// Both add and delete found
									if (shipperData.ShipperQRCode_Data.Any(x => x.Action.ToLower().Contains("delete")) &&
										shipperData.ShipperQRCode_Data.Any(x => x.Action.ToLower().Contains("add"))) status = "r";

									for (int i = 0; i < shipperData.ShipperQRCode_Data.Count(); i++)
										shipperData.ShipperQRCode_Data[i].Status = (shipperData.ShipperQRCode_Data[i].Action.ToLower() == "add") ? status : "a";
											...
											if (IsSuccess == true)
											{
												...
												if (dt == null || dt.Rows.Count == 0 || (dt != null && dt.Rows.Count > 0 && (dt.Rows[0][0] != DBNull.Value ? Convert.ToInt32(dt.Rows[0][0]) : 0) != shipper_Api_Id))
												{
													sqlQuery = "INSERT INTO SHIPPER_QRCODE_API (SHIPPER_QRCODE_API_SYSID, BATCH_NO, MFG_DATE" +
															...
															", TO_DATE(REPLACE('" + DateTime.ParseExact(shipperData.Mfg_Date, "yyMMdd", CultureInfo.InvariantCulture).ToString("dd/MM/yyyy").Replace("-", "/") + "', '-', '/'), 'DD/MM/YYYY')" +
															", TO_DATE(REPLACE('" + DateTime.ParseExact(shipperData.Expiry_Date, "yyMMdd", CultureInfo.InvariantCulture).ToString("dd/MM/yyyy").Replace("-", "/") + "', '-', '/'), 'DD/MM/YYYY')" +
															", SYSDATE, " + plant_id + ", 0, SYSDATE, " + shipperData.ShipperQRCode_Data.Count() + "" +
															",  '" + shipperData.MarketedBy + "', '" + shipperData.ManufacturedBy + "' )";
				...
				else
				{
					...
					Write_Log(Environment.NewLine + $"No any file(s) to upload data. | {DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt").Replace("-", "/")}" + Environment.NewLine, logFilePath);

				}

				Write_Log(Environment.NewLine + $"File Processing Completed at {DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt").Replace("-", "/")}" + Environment.NewLine + Environment.NewLine, logFilePath);
		...
		//									dt = DataContext.ExecuteQuery_SQL($"SELECT SHIPPER_QRCODE_API_SYSID FROM SHIPPER_QRCODE_API WHERE PLANT_ID = {plant_id} AND BATCH_NO = '{shipperData.Batch_no}'");
		...
		//								List<ShipperQRCodeModel> listShipperQRCode = shipperData.ShipperQRCode_Data
		//															.GroupBy(s => new { ShipperQRCode = s.ShipperQRCode, Action = s.Action, BottleQRCode_Len = s.BottleQRCode != null ? s.BottleQRCode.Count() : 0 })
		//															.Select(x =>
		//															{
		//																return new ShipperQRCodeModel() { Id = (long)0, ShipperQRCode = x.Key.ShipperQRCode, Action = x.Key.Action, Count = x.Count(), Length = x.Key.BottleQRCode_Len };
		//															})
		//															.ToList();
		...
		//									while (len <= listShipperQRCode.Count())
		//									{
		//										dt = DataContext.ExecuteQuery_SQL("SELECT SHIPPER_QRCODE_SYSID, shipper_qrcode FROM SHIPPER_QRCODE " +
		//											"WHERE SHIPPER_QRCODE_API_SYSID = " + shipper_Api_Id + " AND LOWER(action) = 'add' " +
		//											"AND shipper_qrcode IN (" + string.Join(", ", listShipperQRCode.Skip(len).Take(500).Select(x => "'" + x.ShipperQRCode + "'").ToArray()) + ")");
		...
		//												if (dr["SHIPPER_QRCODE_SYSID"] != DBNull.Value)
		//													listShipperQRCode[listShipperQRCode.FindIndex(x => x.ShipperQRCode == Convert.ToString(dr["shipper_qrcode"]))].Id = Convert.ToInt64(dr["SHIPPER_QRCODE_SYSID"]);
		...
		//								List<ShipperQRCodeDetails> listBottleQRCode = shipperData.ShipperQRCode_Data.SelectMany(x => x.BottleQRCode)
		//																.GroupBy(qrCode => qrCode)
		//																.Select(qrCode =>
		//																{
		//																	int startIndex = qrCode.Key.IndexOf(")", qrCode.Key.IndexOf(")", 0) + 1);

		//																	if (startIndex >= 0) return new ShipperQRCodeDetails() { Id = (long)0, Shipper_QrCode_Id = (long)0, ShipperQRCode = "", BottleQRCode = qrCode.Key, Count = qrCode.Count(), Length = qrCode.Key.Substring(startIndex + 1, qrCode.Key.Length - startIndex - 1).Length };
		//																	else return new ShipperQRCodeDetails() { Id = (long)0, Shipper_QrCode_Id = (long)0, ShipperQRCode = "", BottleQRCode = qrCode.Key, Count = qrCode.Count(), Length = 0 };
		//																})
		//																//.Where(qrCode => qrCode.Count > 1 || qrCode.Length != 14)
		//																.ToList();
		...
		//									while (len <= listBottleQRCode.Count())
		//									{
		//										dt = DataContext.ExecuteQuery_SQL("SELECT BOTTLE_QRCODE_SYSID, SHIPPER_QRCODE_SYSID, bottle_qrcode FROM bottle_qrcode " +
		//											"WHERE bottle_qrcode IN (" + string.Join(", ", listBottleQRCode.Skip(len).Take(300).Select(x => "'" + x.BottleQRCode + "'").ToArray()) + ")");
		...
		//												if (dr["BOTTLE_QRCODE_SYSID"] != DBNull.Value)
		//												{
		//													var _i = listBottleQRCode.FindIndex(x => x.BottleQRCode == Convert.ToString(dr["bottle_qrcode"]));
		//													listBottleQRCode[_i].Id = Convert.ToInt64(dr["BOTTLE_QRCODE_SYSID"]);
		//													listBottleQRCode[_i].Shipper_QrCode_Id = Convert.ToInt64(dr["SHIPPER_QRCODE_SYSID"]);
		//												}
		//											}
		...
		//									foreach (var item in listBottleQRCode.Where(x => x.Id == 0 || x.Shipper_QrCode_Id == 0).ToList())
		//									{
		//										item.ShipperQRCode = shipperData.ShipperQRCode_Data.Where(x => x.BottleQRCode.Any(z => z.Equals(item.BottleQRCode))).Select(x => x.ShipperQRCode).FirstOrDefault();
		//										item.Shipper_QrCode_Id = listShipperQRCode.Where(x => x.ShipperQRCode == item.ShipperQRCode).Select(x => x.Id).FirstOrDefault();
		//									}
		//								}
		...
		//									dt = DataContext.ExecuteQuery_SQL("SELECT IFNULL(MAX(SHIPPER_QRCODE_SYSID), 0) TEMP_SHIPPER_QRCODE_ID FROM SHIPPER_QRCODE");
		...
		//									for (int i = 0; i < shipperData.ShipperQRCode_Data.Count(); i++)
		//									{
		//										shipperData.ShipperQRCode_Data[i].Id = listShipperQRCode.Where(x => x.ShipperQRCode == shipperData.ShipperQRCode_Data[i].ShipperQRCode).Select(x => x.Id).FirstOrDefault();

		//										if (shipperData.ShipperQRCode_Data[i].Id == 0)
		//										{
		//											shipper_QrCode_Id_Old = 0;
		...
		//											shipperData.ShipperQRCode_Data[i].Id = shipper_QrCode_Id++;
		...
		//									dt = DataContext.ExecuteQuery_SQL("SELECT IFNULL(MAX(BOTTLE_QRCODE_SYSID), 0) TEMP_SHIPPER_QRCODE_ID FROM BOTTLE_QRCODE");
		...
		//									for (int i = 0; i < listBottleQRCode.Count(); i++)
		//									{
		//										if (listBottleQRCode[i].Id == 0)
		//										{
		//											if (productId == 0)
		//												productId = listProduct.Where(y => listBottleQRCode[i].BottleQRCode.Contains(y.GTIN)).Select(y => y.Id).FirstOrDefault();

		//											DataRow newRow = dtBottleQrCode.NewRow();

		//											newRow[0] = bottle_QrCode_Id++;
		//											newRow[1] = listBottleQRCode[i].BottleQRCode;
		//											newRow[2] = productId;
		//											newRow[3] = "a";
		//											newRow[4] = listBottleQRCode[i].Shipper_QrCode_Id;
		//											newRow[5] = plant_id;
		//											newRow[6] = user_id;
		//											newRow[7] = currentDateTime.ToString();
		//											newRow[8] = 0;
		...
		//					if (dt != null && dt.Rows.Count > 0
		//						&& (dt.Rows[0][0] != DBNull.Value ? Convert.ToInt32(dt.Rows[0][0]) : 0) == 0)
		//					{
		//						sqlQuery = "INSERT INTO SHIPPER_QRCODE_API (SHIPPER_QRCODE_API_SYSID, BATCH_NO, MFG_DATE" +
		//							", EXPIRY_DATE, EVENTTIME, PLANT_ID, CREATED_BY, CREATED_DATETIME,  TOTAL_SHIPPER_QTY" +
		//							", MARKETEDBY, MANUFACTUREDBY) " +
		...
		public IActionResult SyncBatch_New_New(string searchTerm = null)
		{
			try
			{
				if (!string.IsNullOrEmpty(searchTerm) && searchTerm.Trim().EndsWith("_Error"))
				...
				if (sourceFilePaths != null && sourceFilePaths.Count() > 0)
				{
					...
					foreach (var sourceFilePath in orderedFilePaths.Where(x => !x.Trim().Contains("_Error") && !destinationFilePaths.Any(y => y.StartsWith(x))).OrderBy(x => x).ToList())
					{
						...
						List<(string QRCode, string BottleQRCodes, string Type)> listShipperQRCode_Duplicate_DB = new List<(string QRCode, string BottleQRCodes, string Type)>();
						...
						try
						{
										...
										&& shipperData.ShipperQRCode_Data.Where(x => x.Action.ToLower().Contains("add")).Count() > 0)
									{
										...
										if (listShipperQRCode != null && listShipperQRCode.Count() > 0)
										{
											...
											while (len <= listShipperQRCode.Count())
											{
												...
												if (dt != null && dt.Rows.Count > 0)
													listShipperQRCode_Duplicate_DB.Add((string.Join(",", dt.AsEnumerable().Select(row => row[0].ToString()).ToArray()), "", "DUP_SHIPPER_DB"));
											...
											for (int i = 0; i < shippers.Count; i += 20)
											{
												...
												if (dtBottleQRCode_ != null && dtBottleQRCode_.Rows.Count > 0)
												{
													var result = groupedResult
														.Where(item => dtBottleQRCode_.AsEnumerable().Any(row => row.Field<string>("bottle_qrcode") == item.BottleQRCode))
														.GroupBy(item => item.ShipperQRCode)  // Group by ShipperQRCode
														.Select(group => (group.Key, string.Join(",", group.Select(item => item.BottleQRCode).Distinct()), "DUP_BOTTLE_DB"))
														.ToList();

													if (result != null && result.Count > 0)
														lock (listShipperQRCode_Duplicate_DB) { listShipperQRCode_Duplicate_DB.AddRange(result); }
												}
											}
										}
										...
										if (shipperData.ShipperQRCode_Data != null && shipperData.ShipperQRCode_Data.Count() > 0)
										{
											DataTable dtAvailable_Shipper = new DataTable();
											DataTable dtAvailable_Bottle = new DataTable();
											DataTable dtShipperQrCode = new DataTable();
											DataTable dtBottleQrCode = new DataTable();
											...
											if (dt != null && dt.Rows.Count > 0 && (dt.Rows[0][0] != DBNull.Value ? Convert.ToInt32(dt.Rows[0][0]) : 0) > 0)
											{
												dt = DataContext.ExecuteQuery_SQL($"SELECT SHIPPER_QRCODE_API_SYSID, DATE_FORMAT(Created_DateTime,'%d-%m-%Y %H:%i')Created_DateTime FROM SHIPPER_QRCODE_API WHERE BATCH_NO = '{shipperData.Batch_no}'");

												if (dt != null && dt.Rows.Count > 0)
												{
													shipper_Api_Id = (dt.Rows[0][0] != DBNull.Value ? Convert.ToInt64(dt.Rows[0][0]) : 0);

													currentDateTime = DateTime.ParseExact(Convert.ToString(dt.Rows[0][1]).Replace("-", "/"), "dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture);
											...
											dtAvailable_Shipper = DataContext.ExecuteQuery_SQL("SELECT PLANT_ID, SHIPPER_QRCODE_SYSID, SHIPPER_QRCODE FROM SHIPPER_QRCODE " +
															$"WHERE PLANT_ID = {plant_id} AND SHIPPER_QRCODE_API_SYSID = {shipper_Api_Id}");

											dtAvailable_Bottle = DataContext.ExecuteQuery_SQL("SELECT PLANT_ID, BOTTLE_QRCODE_SYSID, BOTTLE_QRCODE FROM BOTTLE_QRCODE " +
														$"WHERE PLANT_ID = {plant_id} AND SHIPPER_QRCODE_SYSID " +
														$"IN (SELECT SHIPPER_QRCODE_SYSID FROM SHIPPER_QRCODE " +
														$"WHERE PLANT_ID = {plant_id} AND SHIPPER_QRCODE_API_SYSID = {shipper_Api_Id})");
											...
											if (IsSuccess == true)
											{
												...
												for (int i = 0; i < shipperData.ShipperQRCode_Data.Count(); i++)
												{
													...
													if (dtAvailable_Shipper != null && dtAvailable_Shipper.Rows.Count > 0
														&& dtAvailable_Shipper.AsEnumerable()
															.Any(row => row.Field<string>("SHIPPER_QRCODE") == shipperData.ShipperQRCode_Data[i].ShipperQRCode))
													{
														shipperData.ShipperQRCode_Data[i].Id = (from row in dtAvailable_Shipper.AsEnumerable()
																								where row.Field<string>("SHIPPER_QRCODE") == shipperData.ShipperQRCode_Data[i].ShipperQRCode
																								select row.Field<Int64>("SHIPPER_QRCODE_SYSID")).FirstOrDefault();
													}
												...
												for (int x = 0; x < shipperData.ShipperQRCode_Data.Count(); x++)
												{
													...
													for (int i = 0; i < shipperData.ShipperQRCode_Data[x].BottleQRCode.Count(); i++)
													{
														...
														if (dtAvailable_Bottle != null && dtAvailable_Bottle.Rows.Count > 0
												...
												if (dtShipperQrCode != null && dtShipperQrCode.Rows.Count > 0)
												{
													...
													while (startIndex < dtShipperQrCode.Rows.Count)
													{
														...
														var rowsInSqlNotInOracle = from row2 in nextBatch.AsEnumerable()
																				   join row1 in dtAvailable_Shipper.AsEnumerable()
																				   on new { SHIPPER_QRCODE = row2.Field<string>("SHIPPER_QRCODE") }
																				   equals new { SHIPPER_QRCODE = row1.Field<string>("SHIPPER_QRCODE") }
																				   into gj
																				   from subRow in gj.DefaultIfEmpty()
												...
												if (dtBottleQrCode != null && dtBottleQrCode.Rows.Count > 0)
												{
													...
													for (int chunkIndex = 0; chunkIndex < numberOfChunks; chunkIndex++)
													{
														...
														for (int subChunkIndex = 0; subChunkIndex < numberOfSubChunks; subChunkIndex++)
														{
															...
															var rowsInSqlNotInOracle = from row2 in subChunk.AsEnumerable()
																					   join row1 in dtAvailable_Bottle.AsEnumerable()
																					   on new { BOTTLE_QRCODE = row2.Field<string>("BOTTLE_QRCODE") }
																					   equals new { BOTTLE_QRCODE = row1.Field<string>("BOTTLE_QRCODE") }
																					   into gj
																					   from subRow in gj.DefaultIfEmpty()
											...
											if (/*IsSuccess ==*/ true)
											{
												...
												dtAvailable_Shipper = DataContext.ExecuteQuery("SELECT PLANT_ID, SHIPPER_QRCODE_SYSID, SHIPPER_QRCODE FROM SHIPPER_QRCODE " +
																$"WHERE PLANT_ID = {plant_id} AND SHIPPER_QRCODE_API_SYSID = {shipper_Api_Id}");

												dtAvailable_Bottle = DataContext.ExecuteQuery("SELECT PLANT_ID, BOTTLE_QRCODE_SYSID, BOTTLE_QRCODE FROM BOTTLE_QRCODE " +
															$"WHERE PLANT_ID = {plant_id} AND SHIPPER_QRCODE_SYSID " +
															$"IN (SELECT SHIPPER_QRCODE_SYSID FROM SHIPPER_QRCODE " +
															$"WHERE PLANT_ID = {plant_id} AND SHIPPER_QRCODE_API_SYSID = {shipper_Api_Id})");
							...
							if (!string.IsNullOrEmpty(Convert.ToString(fileContent)))
							{
								...
								try
								{
									...
									destinationFilePath = Path.Combine(destinationFolderPath, $"{fileNameWithoutExtension}_{currentDateTime.ToString("yyyyMMdd_HHmmsss")}{fileExtension}");
									errorFilePath = Path.Combine(errorFolderPath, $"{fileNameWithoutExtension}_{currentDateTime.ToString("yyyyMMdd_HHmmsss")}_Error{fileExtension}");
						...
						if (!string.IsNullOrEmpty(error))
						{
							...
							try
							{
								...
								var query_File = $"INSERT INTO SHIPPER_QR_CODE_FILE_UPLOAD_STATUS (FILEUPLOADNAME, STARTDATE, ENDDATE, QRCODECOUNT, TOTAL_SHIPPER_QTY, ACCEPTED_SHIPPER_QTY, REJECTED_SHIPPER_QTY, FILESTATUS, REMARK) " +
															$"VALUES ( '{fileName.Substring(0, fileName.Length - (fileName.Length - fileName.LastIndexOf('.')))}'" +
															$", STR_TO_DATE('{currentDateTime.ToString("dd-MM-yyyy HH:mm").Replace("-", "/")}', '%d/%m/%Y %H:%i')" +
															$", STR_TO_DATE('{currentDateTime.ToString("dd-MM-yyyy HH:mm").Replace("-", "/")}', '%d/%m/%Y %H:%i'), 0, 0, 0, 0, '{fileUploadStatus}', '{error}' );";
				...
				if (shipper_QrCode_Api_Id > 0)
				{
					try
					{
						...
						if (true)
						{
							...
							var rowsNotInOracle = from shipper in dtBottleQrCode.AsEnumerable()
		...
		public IActionResult SyncBatch_Oracle_SQL(string batch_no = "")
		{
			try
			{
				string plantCode = Convert.ToString(AppHttpContextAccessor.AppConfiguration.GetSection("PlantCode").Value ?? "");
